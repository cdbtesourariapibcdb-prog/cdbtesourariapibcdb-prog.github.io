/****************************************************
 * dashboard.js — Gráficos e Resumo · Tesouraria CDB
 ****************************************************/
...
function renderFromData(res) {
  ...
  const labels = months.map((k) => {
    const [y, m] = k.split("-");
    return new Date(parseInt(y), parseInt(m) - 1, 1).toLocaleString("pt-BR", {
      month: "short",
      year: "numeric",
    });
  });
  ...
  document.getElementById("totalEntradas").textContent = fmtBRL(
    dataE.reduce((a, b) => a + b, 0)
  );
  document.getElementById("totalSaidas").textContent = fmtBRL(
    dataS.reduce((a, b) => a + b, 0)
  );
  const saldo = dataE.reduce((a, b) => a + b, 0) - dataS.reduce((a, b) => a + b, 0);
  const el = document.getElementById("saldoFinal");
  el.textContent = fmtBRL(saldo);
  el.style.color = saldo >= 0 ? "#0b9b3a" : "#d23b3b";
  ...
  if (chartPieE) chartPieE.destroy();
  if (chartPieS) chartPieS.destroy();
...
async function loadDashboard() {
  try {
    ...
    document.getElementById("monthlyTable").innerHTML =
      "<p class='small'>Erro ao carregar gráficos.</p>";
  }
}
